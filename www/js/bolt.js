...
/***/ function(module, exports, __webpack_require__) {
	...
	(function() {
	  ...
	  var Chartist = __webpack_require__(4);
...
/***/ function(module, exports) {
	...
	function gradientDescent(alpha, x, y, epsilon, max_iter) {
	  epsilon = typeof epsilon !== 'undefined' ?  epsilon : 0.0001;
	  max_iter = typeof max_iter !== 'undefined' ?  max_iter : 10000;
	  ...
	  var arr = [];
	  for (i = 0; i < m; i++) {
	    arr.push(Math.pow((t0 + t1*x[i] - y[i]), 2));
	  }
	  J = sum(arr)/m;
	  ...
	  while (!converged) {
	    ...
	    var grad0, grad1,
	        d0 = [], d1 = [];
	    for (i = 0; i < m; i++) {
	      d0.push((t0 + t1*x[i] - y[i]));
	    }
	    grad0 = 1.0/m * sum(d0);
	    for (i = 0; i < m; i++) {
	      d1.push((t0 + t1*x[i] - y[i])*x[i]);
	    }
	    grad1 = 1.0/m * sum(d1);
	    ...
	    var e = [];
	    for (i = 0; i < m; i++) {
	      e.push(Math.pow((t0 + t1*x[i] - y[i]), 2));
	    }
	    err = sum(e);

	    if (Math.abs(J-err) <= epsilon) {
	      console.log('Converged, iterations: ' + iter + ' !!!');
	      converged = true;
	    }
	    ...
	    if (iter == max_iter) {
	      console.log('Max interactions exceeded!');
	      converged = true;
	    } 
